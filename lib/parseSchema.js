...
(function(){
    ...
    exports.generateDocFromSwagger = function generateDocFromSwagger(inputFilePath, url, method, options, genFunc) {
		...
		_.each(keyArr, function(key) {
			...
			finaloutput = finaloutput + ' \r\n ' + title + buildStr((_.get(finalObj.fields, key.key)).properties, genFunc === undefined ? alternateFunc.generateFunction : genFunc, spacesToAdd, options.serializer || '');
...
exports.generateDocFromJson = function generateDocFromSwagger(inputFile, key, options, genFunc) {
	...
	var finaloutput = '';
	 ...
	 finaloutput = finaloutput + ' \n ' + buildStr((_.get(obj, key)).properties, genFunc, spacesToAdd, options.serializer || '');
...
function buildStr(obj, genFunc, spacesToAdd, serializer) {
	var formattedStr = '';
	_.each(obj, function(property) {
			
		if (property.type === 'object' || property.type === 'array') {
			formattedStr = formattedStr +  ' \r\n ' + spacesToAdd + genFunc(property, serializer);
			if (property.type === 'array') {
				
				if (property.itemType !== undefined) {
					formattedStr = formattedStr  +spacesToAdd  + ', ' + genFunc(property.itemType, serializer) + ')';
					
				} else {
					var origTab = spacesToAdd;
					spacesToAdd = spacesToAdd + space2; //' \t ';
					formattedStr = formattedStr + ' \r\n ' + spacesToAdd + '['; //tabsToAdd + //'\r\n' + 
					formattedStr = formattedStr + spacesToAdd + buildStr(property.properties, genFunc, spacesToAdd + space2 + space2, serializer); //' \t \t ');
					formattedStr = formattedStr + ' \r\n ' + spacesToAdd  + '] \r\n ' + spacesToAdd + ')';
					spacesToAdd = origTab;
				}
				
			} else {
				formattedStr = formattedStr + spacesToAdd + '{' + spacesToAdd  +  buildStr(property.properties, genFunc, spacesToAdd + space2 + space2 + space2, serializer) + '\r\n' + spacesToAdd + space2 + space2 + ' ' +'}';
...
function processRef (item, itemObj) {
	...
	if (_.startsWith(propRef, 'http')) {
		...
		if (_.get(fullInput.definitions, ref).type !== undefined) {
			...
			itemObj.reference = ref;
			itemObj.properties = processProperties(_.get(fullInput.definitions, ref).properties, _.get(fullInput.definitions, ref).required, _.get(fullInput.definitions, ref).discriminator, _.get(fullInput.definitions, ref).readOnly);
		}
